...
RE_BRACKETS = re.compile('|'.join(' ?%s.*?%s' % (re.escape(
    brackets[i]), re.escape(brackets[i + 1])) for i in range(0, len(brackets), 2)))
...
notchinese = lambda l: not l or sum(
    (ord(i) not in ucjk) for i in l) > .5 * len(l)
brcksub = lambda matchobj: '' if notchinese(
    matchobj.group(0)[1:-1]) else matchobj.group(0)
